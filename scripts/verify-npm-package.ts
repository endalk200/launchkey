...
const npmCache = join(tmpdir(), "launchkey-npm-cache");
...
if (packageJson.private === true) {
	throw new Error("@launchkey/cli must not be private when preparing the npm package.");
}
...
if (presentDependencyFields.length > 0) {
	throw new Error(
		`@launchkey/cli is expected to publish without runtime npm dependencies; found ${presentDependencyFields.join(", ")}.`,
	);
}
...
console.log(`Verified @launchkey/cli@${packedPackage.version} package contents.`);
