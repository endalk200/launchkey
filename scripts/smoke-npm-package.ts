...
const smokeRoot = await mkdtemp(join(tmpdir(), "launchkey-cli-smoke-"));
...
const binPath = join(smokeRoot, "node_modules", ".bin", "launchkey");
const actualVersion = run(binPath, ["version"], smokeRoot);

if (actualVersion !== packageJson.version) {
	throw new Error(`Expected launchkey version to print ${packageJson.version}, got ${actualVersion}.`);
}

const flagVersion = run(binPath, ["--version"], smokeRoot);

if (!flagVersion.includes(packageJson.version)) {
	throw new Error(`Expected launchkey --version to include ${packageJson.version}, got ${flagVersion}.`);
}

const configPath = run(binPath, ["config", "path"], smokeRoot);

if (!configPath.endsWith("/.launchkey/config.toml")) {
	throw new Error(`Expected config path smoke test to print the default config path, got ${configPath}.`);
}

console.log(`Smoke-tested @launchkey/cli@${packageJson.version} from ${packedPackage.filename}.`);
