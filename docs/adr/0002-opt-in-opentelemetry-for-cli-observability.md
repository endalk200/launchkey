...
Status: Amended by [0003 Shared LaunchKey Configuration Package](./0003-shared-launchkey-configuration-package.md)

LaunchKey's CLI exports observability data only when `LAUNCHKEY_TELEMETRY=true`.
...
default, sending traces to `/v1/traces` and logs to `/v1/logs`. LaunchKey Users can
override the base endpoint with `LAUNCHKEY_OTLP_ENDPOINT` for collectors that bind
...
and should not export logs or traces unless the LaunchKey User asks for it. We also
...
from LaunchKey Configuration at `~/.launchkey/config.toml`. Environment variables
continue to override file configuration, and `LAUNCHKEY_TELEMETRY` remains the
