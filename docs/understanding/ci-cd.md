...
This document explains how LaunchKey checks changes and releases the CLI package.
...
| `Stage npm Release` | Pushes to `main`, manual dispatch | Stages a new `@launchkey/cli` package version on npm. |
...
LaunchKey publishes one public package: `@launchkey/cli`. Internal workspace packages
...
9. creates tag `launchkey-cli-vX.Y.Z` and a draft GitHub Release
...
The repository currently records `@launchkey/cli` version `0.2.0` in
`apps/cli/package.json`. npm currently has the bootstrap package
`@launchkey/cli@0.1.0` published. The next automated release from this
repository is intended to stage and publish `@launchkey/cli@0.2.0`.

The release tag format is `launchkey-cli-vX.Y.Z`. Tags matching
`launchkey-cli-v*` are documented as protected from deletion and non-fast-forward
updates.
...
- protected release tags for `launchkey-cli-v*`
