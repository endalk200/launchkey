...
import {
	...
	initLaunchKeyConfigFromEnvironment,
	LaunchKeyConfig,
	loadLaunchKeyConfigFromEnvironment,
	OTLP_ENDPOINT_ENV,
	parseTelemetryEnabledEnv,
	parseTelemetryEndpointEnv,
	resolveConfigPath,
	TELEMETRY_ENV,
	validateLaunchKeyConfigFromEnvironment,
} from "./index.js";
...
describe("@launchkey/config", () => {
	it.effect("loads built-in defaults when the default config file is missing", () =>
		Effect.gen(function* () {
			const config = yield* loadLaunchKeyConfigFromEnvironment({});
	...
	it.effect("uses env overrides before TOML file values", () =>
		Effect.gen(function* () {
			const config = yield* loadLaunchKeyConfigFromEnvironment({
				[CONFIG_PATH_ENV]: "/tmp/launchkey-config-test.toml",
			...
			Effect.provide(
				fileSystemLayer({
					"/tmp/launchkey-config-test.toml": `[telemetry]
enabled = true
...
`,
	...
	it.effect("keeps LAUNCHKEY_TELEMETRY strict", () =>
		Effect.gen(function* () {
	...
	it.effect("parses LAUNCHKEY_OTLP_ENDPOINT as an absolute URL override", () =>
		Effect.gen(function* () {
	...
	it.effect("reports invalid file and env sources separately", () =>
		Effect.gen(function* () {
			const report = yield* validateLaunchKeyConfigFromEnvironment({
				[CONFIG_PATH_ENV]: "/tmp/launchkey-invalid-config-test.toml",
				[TELEMETRY_ENV]: "1",
			});
			...
			Effect.provide(
				fileSystemLayer({
					"/tmp/launchkey-invalid-config-test.toml": `[telemetry]
enabled = true
...
`,
	...
	it.effect("does not hide an invalid file source behind valid env overrides during validation", () =>
		Effect.gen(function* () {
			const report = yield* validateLaunchKeyConfigFromEnvironment({
				[CONFIG_PATH_ENV]: "/tmp/launchkey-invalid-lower-precedence.toml",
				[OTLP_ENDPOINT_ENV]: "http://127.0.0.1:4318",
			});
			...
			Effect.provide(
				fileSystemLayer({
					"/tmp/launchkey-invalid-lower-precedence.toml": `[telemetry]
enabled = true
otlp_endpoint = "not-a-url"
`,
				}),
	...
	it.effect("provides the resolved service through an Effect layer", () =>
		...
		}).pipe(Effect.provide(LaunchKeyConfig.layerFromEnvironment({})), Effect.provide(fileSystemLayer({}))),
	);

	it.effect("initializes a starter config file without overwriting", () =>
		Effect.gen(function* () {
			const files: Record<string, string> = {};
			const env = { [CONFIG_PATH_ENV]: "/tmp/launchkey-init-config-test.toml" };
			const path = yield* initLaunchKeyConfigFromEnvironment(env).pipe(Effect.provide(fileSystemLayer(files)));

			assert.strictEqual(path.path, "/tmp/launchkey-init-config-test.toml");
			assert.include(files["/tmp/launchkey-init-config-test.toml"] ?? "", "[telemetry]");

			const alreadyExists = yield* Effect.flip(
				initLaunchKeyConfigFromEnvironment(env).pipe(Effect.provide(fileSystemLayer(files))),
			);
	...
	it.effect("maps config init filesystem failures to config write errors", () =>
		Effect.gen(function* () {
			const path = "/tmp/launchkey-init-write-failure.toml";
			...
			const failure = yield* Effect.flip(
				initLaunchKeyConfigFromEnvironment({ [CONFIG_PATH_ENV]: path }).pipe(
					Effect.provide(
						FileSystem.layerNoop({
