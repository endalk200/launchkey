...
export const DEFAULT_CONFIG_PATH = "~/.launchkey/config.toml";
export const CONFIG_PATH_ENV = "LAUNCHKEY_CONFIG_PATH";
export const TELEMETRY_ENV = "LAUNCHKEY_TELEMETRY";
export const OTLP_ENDPOINT_ENV = "LAUNCHKEY_OTLP_ENDPOINT";
...
export type LaunchKeyConfiguration = {
	readonly telemetry: TelemetryConfig;
};

export const defaultLaunchKeyConfiguration: LaunchKeyConfiguration = {
	telemetry: {
		enabled: false,
...
export type ConfigValidationReport = {
	...
	readonly config: LaunchKeyConfiguration | undefined;
};
...
export const telemetryConfigDescriptor = Config.all({
	enabled: Config.boolean("enabled").pipe(Config.withDefault(defaultLaunchKeyConfiguration.telemetry.enabled)),
	otlpEndpoint: Config.url("otlp_endpoint").pipe(
		Config.withDefault(new URL(defaultLaunchKeyConfiguration.telemetry.otlpEndpoint)),
...
export const starterConfigToml = `# LaunchKey Configuration
#
	...
	Effect.suspend(() => {
		...
		Effect.tap((path) =>
			Effect.annotateCurrentSpan({
				"launchkey.config.path": path.path,
				"launchkey.config.path_source": path.source,
			}),
		),
		Effect.tap((path) =>
...
export const loadLaunchKeyConfigFromEnvironment = (env: Record<string, string | undefined>) =>
	Effect.gen(function* () {
		const sources = yield* resolveConfigSources(env);
		return yield* parseEffectiveConfig(sources);
	});

export const loadLaunchKeyConfig = Effect.gen(function* () {
	const sources = yield* resolveConfigSources(process.env);
	return yield* parseEffectiveConfig(sources);
});

export const validateLaunchKeyConfigFromEnvironment = (env: Record<string, string | undefined>) =>
	Effect.gen(function* () {
		const sources = yield* resolveConfigSources(env);

		return yield* validateResolvedConfigSources(sources);
	}).pipe(Effect.withSpan("launchkey.config.validate.from_env"));

export const validateLaunchKeyConfig = Effect.gen(function* () {
	const sources = yield* resolveConfigSources(process.env);

	return yield* validateResolvedConfigSources(sources);
}).pipe(Effect.withSpan("launchkey.config.validate"));
	...
	Effect.gen(function* () {
		...
		const report = {
			...
			effective: Result.isSuccess(effectiveResult)
				? validStatus("Valid LaunchKey Configuration.")
...
export const initLaunchKeyConfigFromEnvironment = (env: Record<string, string | undefined>) =>
	Effect.gen(function* () {
		const path = yield* resolveConfigPath(env);
		return yield* initLaunchKeyConfigAtPath(path);
	});

export const initLaunchKeyConfig = Effect.gen(function* () {
	const path = yield* resolveConfigPath(process.env);

	return yield* initLaunchKeyConfigAtPath(path);
});

const initLaunchKeyConfigAtPath = (path: ConfigPathResolution) =>
	Effect.gen(function* () {
		...
		yield* Effect.annotateCurrentSpan({
			"launchkey.config.exists": exists,
		});
		...
		yield* Effect.logInfo("Created LaunchKey Configuration file", {
			path: path.path,
			source: path.source,
		});

		yield* Effect.annotateCurrentSpan({
			"launchkey.config.created": true,
		});

		return path;
	}).pipe(
		Effect.withSpan("launchkey.config.write_file", {
			attributes: {
				"launchkey.config.parent_directory": dirname(path.path),
				"file.operation": "write",
			},
		}),
	);

export class LaunchKeyConfig extends Context.Service<LaunchKeyConfig, LaunchKeyConfiguration>()("LaunchKeyConfig") {
	static readonly layer = Layer.effect(LaunchKeyConfig)(loadLaunchKeyConfig);
	static readonly layerFromEnvironment = (env: Record<string, string | undefined>) =>
		Layer.effect(LaunchKeyConfig)(loadLaunchKeyConfigFromEnvironment(env));
}
...
const loadFileProvider = (
	...
	Effect.gen(function* () {
		...
		yield* Effect.annotateCurrentSpan({
			"launchkey.config.file_present": exists,
		});
		...
		if (!exists) {
			...
			yield* Effect.annotateCurrentSpan({
				"launchkey.config.file_presence": "missing_default",
			});
			return {
				_tag: "missingDefault",
		...
		yield* Effect.annotateCurrentSpan({
			"launchkey.config.file_presence": "present",
		});
...
const loadEnvOverrideProvider = (
	...
	Effect.gen(function* () {
		...
		yield* Effect.annotateCurrentSpan({
			"launchkey.telemetry.env_present": env[TELEMETRY_ENV] !== undefined,
			"launchkey.telemetry.endpoint_env_present": env[OTLP_ENDPOINT_ENV] !== undefined,
		});
	...
	}).pipe(Effect.withSpan("launchkey.config.load_env"));
...
const parseEffectiveConfig = (sources: ResolvedConfigSources): Effect.Effect<LaunchKeyConfiguration, ConfigError> =>
	Effect.gen(function* () {
		...
		const provider = ConfigProvider.orElse(
			env.provider,
			ConfigProvider.orElse(file.provider, ConfigProvider.fromUnknown(defaultLaunchKeyConfiguration)),
		);

		const config = yield* launchKeyConfigDescriptor.parse(provider);

		yield* Effect.annotateCurrentSpan({
			"launchkey.config.effective_status": "valid",
			"launchkey.telemetry.enabled": config.telemetry.enabled,
		});

		return config;
	}).pipe(Effect.withSpan("launchkey.config.parse_effective"));
