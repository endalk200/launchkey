...
const launchKeyEnvKeys = [CONFIG_PATH_ENV, TELEMETRY_ENV, OTLP_ENDPOINT_ENV] as const;

const withIsolatedLaunchKeyEnvironment = <A, E, R>(effect: Effect.Effect<A, E, R>) =>
	Effect.suspend(() => {
		const previous = Object.fromEntries(launchKeyEnvKeys.map((key) => [key, process.env[key]]));

		for (const key of launchKeyEnvKeys) {
...
const runLaunchKeyCommand = (args: ReadonlyArray<string>, files: Record<string, string> = {}) =>
	Effect.gen(function* () {
	...
	}).pipe(withIsolatedLaunchKeyEnvironment, Effect.provide(cliTestLayer(files)));

describe("launchkey CLI", () => {
	it.effect("prints root help and succeeds when invoked without arguments", () =>
		Effect.gen(function* () {
			const { stdout } = yield* runLaunchKeyCommand([]);
			const stdoutText = stdout.join("\n");

			assert.include(stdoutText, "launchkey <subcommand> [flags]");
	...
	it.effect("prints the package version with the version command", () =>
		Effect.gen(function* () {
			const { stdout } = yield* runLaunchKeyCommand(["version"]);
	...
	it.effect("prints the effective config path", () =>
		Effect.gen(function* () {
			const { stdout } = yield* runLaunchKeyCommand(["config", "path"]);
			const normalizedPath = String(stdout[0] ?? "").replaceAll("\\", "/");

			assert.strictEqual(normalizedPath.endsWith("/.launchkey/config.toml"), true);
		}),
	);

	it.effect("validates the missing default config with defaults", () =>
		Effect.gen(function* () {
			const { stdout } = yield* runLaunchKeyCommand(["config", "validate"]);
			const stdoutText = stdout.join("\n");

			assert.include(stdoutText, "No config file found");
			assert.include(stdoutText, "Environment overrides are valid");
			assert.include(stdoutText, "Valid LaunchKey Configuration.");
		}),
	);
	...
	it.effect("formats config validation output and failure policy in one place", () =>
		Effect.sync(() => {
			const report: ConfigValidationReport = {
				path: { path: "/tmp/launchkey.toml", source: "env" },
	...
	it.effect("keeps telemetry env parsing in LaunchKey Configuration", () =>
		Effect.gen(function* () {
	...
	it.effect("builds disabled telemetry from resolved LaunchKey Configuration", () =>
		Effect.gen(function* () {
	...
	it.effect("prints CLI failures to stderr through the failure reporting module", () =>
		Effect.gen(function* () {
			...
			assert.deepStrictEqual(stderr, ['Invalid LAUNCHKEY_CONFIG_PATH value "". Expected a non-empty path.']);
		}).pipe(Effect.provide(TestConsole.layer)),
	);
});
