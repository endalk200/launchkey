...
const traceCliRun = <E, R>(args: ReadonlyArray<string>, effect: Effect.Effect<void, E, R>) =>
	Effect.suspend(() => {
		...
		return Effect.gen(function* () {
			yield* Effect.logInfo("LaunchKey CLI command started", { args, version: VERSION });
			yield* effect;
		}).pipe(
			Effect.withSpan(spanName, {
				attributes: {
					"cli.command": commandNameFromRootSpan(spanName),
					"launchkey.cli.args": args.join(" "),
					"launchkey.cli.version": VERSION,
				},
			}),
		);
	});
